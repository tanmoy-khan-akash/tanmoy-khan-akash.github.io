...
function initGithubModal() {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && githubModalShown) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initGithubModal);
} else {
  initGithubModal();
}
...

{
  ...
  // Single event handler for reset button that does both actions
  const resetBtn = document.querySelector("#board-reset") ?? assertNonNull()
  resetBtn.addEventListener("click", () => {
    // Reset the board first
    setBoard(defaultBoard);
    // Then show the GitHub modal after a small delay
    setTimeout(showGithubFollowPrompt, 100);
  })
}
